...
class Curve {
  ...
  Curve(int pn, float r) {
    n=pn; 
    declarePoints(); 
    resetPoints(r);
  }
  Curve(int pn, float r, pt Q) {
    n=pn; 
    declarePoints(); 
    resetPoints(r, Q);
  }
  ...
  void resetPoints() {
    float r=100; 
    for (int i=0; i<n; i++) {
      P[i].x=r*cos(TWO_PI/n); 
      P[i].y=r*sin(TWO_PI/n);
    };
  } // init the points to be on a circle
  void resetPoints(float r) {
    println(">>n="+n); 
    for (int i=0; i<n; i++) {
      P[i].x=r*cos(TWO_PI/n*i); 
      P[i].y=r*sin(TWO_PI/n*i);
    };
  } // init the points to be on a circle
  ...
  void delete() { 
    for (int i=p; i<n-1; i++) P[i].set(P[next(i)]); 
    n--; 
    p=prev(p);
  }
  ...
  void insert(pt M) {                // grabs closeest vertex or adds vertex at closest edge. It will be dragged by te mouse
    p=0; 
    for (int i=0; i<n; i++) if (d(M, P[i])<d(M, P[p])) p=i; 
    int e=-1;
    float d = d(M, P[p]);
    for (int i=0; i<n; i++) {
      float dd=d(M, P(P[i], M, P[next(i)])); 
      if (dd<d) {
        e=i; 
        d=dd;
      };
    ...
    if (e!=-1) { 
      for (int i=n-1; i>e; i--) P[i+1].set(P[i]); 
      n++; 
      p=next(e); 
      P[p].set(M);
    };
  ...
  Curve makeFrom(Curve C, int v) {
    empty(); 
    for (float t=0; t<=1.001; t+=.1/v) append(C.interpolate(t)); 
    return this;
  }
  pt interpolate(float t) { 
    return D(P[0], P[1], P[2], P[3], P[4], t);
  }

  Curve drawEdges() {
    beginShape(); 
    for (int i=0; i<n; i++) vertex(P[i]); 
    endShape(); 
    return this;
  }  // fast draw of edges
  Curve showAll() {
...
//    showSamples();
  ...
  float distanceTo(pt M) {
    float md=d(M, P[0]); 
    for (int i=1; i<n; i++) md=min(md, d(M, P[i])); 
    return md;
  }
  void savePts() {
    savePts("data/P.pts");
  ...
  void savePts(String fn) { 
    ...
    inppts[s++]=str(n); 
    for (int i=0; i<n; i++) {
      inppts[s++]=str(P[i].x)+","+str(P[i].y)+","+str(P[i].z);
    };
  ...
  void addDif(Curve R, Curve C) {
    for (int i=0;i<n; i++) P[i].add(V(R.pt(i), C.pt(i)));
  }    
  float length () {
    float L=0; 
    for (int i=0; i<n; i++) L+=d(P[i], P[next(i)]);  
    return(L);
  }    
  ...
  Curve resampleDistance(float r) { 
    ...
    while (go) {
      int j=i; 
      while (j<n && d (P[j+1], C)<r) j++; // last vertex in sphere(C,r);
      if (j>=n-1) go=false; 
      else {
        pt A=P[j], B=P[j+1]; 
  ...
  Curve computeFirstTwistFreeNormal() {
    ...
    vec VxI = N(V, I); 
    vec VxJ = N(V, J); 
    if (VxI.norm() > VxJ.norm()) Nx[0]=U(N(VxI, V)); 
    else Nx[0]=U(N(VxJ, V));
    return this;
  }
  ...
  Curve computeTwistFreeNormals() {
    pt Q=P(P[0], 50, Nx[0]);
    for (int i=1; i<n-1; i++) {
      vec BD=V(P[i-1], P[i+1]); 
      float s=d(V(Q, P[i]), BD)/d(V(P[i-1], P[i]), BD); 
      Q=P(Q, s, V(P[i-1], P[i])); 
      Nx[i]=U(P[i], Q);
    }
    Nx[n-1]=Nx[n-2];
  ...
  void showTube(float r, int ne, int nq, color col) {
    ...
    int p=0; 
    for (int j=0; j<ne; j++) C[p][j]=P( P[0], c[j], Nx[0], s[j], Ny[0] ); 
    p=1-p;
    noStroke();
    for (int i=1; i<n-1; i++) {
      if (i%nq==0) dark=!dark; 
      for (int j=0; j<ne; j++) C[p][j]=P( P[i], c[j], Nx[i], s[j], Ny[i]); 
      p=1-p;
      if (i>0) for (int j=0; j<ne; j++) {
        if (dark) fill(200, 200, 200); 
        else fill(col); 
        dark=!dark; 
        int jp=(j+ne-1)%ne; 
        beginShape(); 
