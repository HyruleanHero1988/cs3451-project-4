...
void draw() {  
  ...
  rotateY(PI/2); 
  rect(0, 0, 400, 400); 
  popMatrix(); // displays surface of model 1 to set the z-buffer for picking points Q and T and for picking the local frames 1 and 2
  ...
  noStroke(); 
  show(Q, 30, I, J, K); // shows local frame aligned with screen when picked ('q' pressed) using (R,G,B) lines
  noStroke(); 
  fill(cyan); 
  show(T, 2); // shows picked point in cyan (it is set when 't' is pressed and becomes the focus) 
  fill(red); 
  show(mQ[1], 3); 
  fill(green); 
  show(mQ[2], 3); // shows origin of frame 1 and 2 as a small red or green ball
  if (m==1) fill(red); 
  else if (m==2) fill(green); 
  else if (m==0) fill(blue); 
  show(mQ[m], 5); // shows origin of selected frame (R,G,B) for (1,2,0) as a bigger ball
  fill(yellow); 
  show(Q, 2); // shows current point on surface. Changed when 'q' is pressed
  noStroke(); 
  showModel(); // shows second model (currently axes)
