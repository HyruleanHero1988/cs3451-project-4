...
vec V(vec A, vec B) {
  return new vec((A.x+B.x)/2.0, (A.y+B.y)/2.0, (A.z+B.z)/2.0);
}                      // (A+B)/2
vec V(vec A, float s, vec B) {
  return new vec(A.x+s*(B.x-A.x), A.y+s*(B.y-A.y), A.z+s*(B.z-A.z));
};      // (1-s)A+sB
vec V(vec A, vec B, vec C) {
  return new vec((A.x+B.x+C.x)/3.0, (A.y+B.y+C.y)/3.0, (A.z+B.z+C.z)/3.0);
};  // (A+B+C)/3
...
vec V(float a, vec A, float b, vec B) {
  return A(V(a, A), V(b, B));
}                                       // aA+bB 
vec V(float a, vec A, float b, vec B, float c, vec C) {
...
class pt { 
  ...
  pt add(float dx, float dy, float dz) {
    x+=dx; 
    y+=dy; 
    z+=dz; 
    return this;
  ...
  pt mul(float dx, float dy, float dz) {
    x*=dx; 
    y*=dy; 
    z*=dz; 
    return this;
  ...
  pt coord2d(){
    return P(screenX(this.x, this.y, this.z),screenY(this.x, this.y, this.z),0); 
  }
...
float d2(pt P, pt Q) {
  return sq(Q.x-P.x)+sq(Q.y-P.y)+sq(Q.z-P.z);
};                                 // AB^2 distance squared
float m(vec U, vec V, vec W) {
  return d(U, N(V, W));
...
void showShrunkOffset(pt A, pt B, pt C, float e, float h) {
  vec N=U(N(V(A, B), V(A, C))); 
  showShrunk(P(A, h, N), P(B, h, N), P(C, h, N), e);
} // offset by h along normal
...
// INTERPOLATING CURVE
void drawCurve(pt A, pt B, pt C, pt D) {
  float d=d(A, B)+d(B, C)+d(C, D); 
  beginShape(); 
  for (float t=0; t<=1; t+=0.025) vertex(P(A, B, C, D, t*d)); 
  endShape();
}
void drawSamplesOnCurve(pt A, pt B, pt C, pt D, float r) {
  float d=d(A, B)+d(B, C)+d(C, D); 
  for (float t=0; t<=1; t+=0.025) show(P(A, B, C, D, t*d), r);
}
pt P(pt A, pt B, pt C, pt D, float t) {
  return P(0, A, d(A, B), B, d(A, B)+d(B, C), C, d(A, B)+d(B, C)+d(C, D), D, t);
}
...
pt L4(pt A, pt B, pt C, pt D, float t){ //t is 0-1 between B and C
  pt pOne = L3(A,B,C,1+t);
  pt pTwo = L3(B,C,D,t);
  return P(pOne, pTwo); //P(pt, pt) is the midpoint formula
}


// Interpolation non-uniform (Neville's algorithm)
pt I(float a, pt A, float b, pt B, float t) {
  return L(A, B, (t-a)/(b-a));
}                               // P(a)=A, P(b)=B
pt I(float a, pt A, float b, pt B, float c, pt C, float t) {
  pt P=I(a, A, b, B, t); 
  pt Q=I(b, B, c, C, t); 
  return I(a, P, c, Q, t);
} // P(a)=A, P(b)=B, P(c)=C
pt I(float a, pt A, float b, pt B, float c, pt C, float d, pt D, float t) {
  pt P=I(a, A, b, B, c, C, t); 
  pt Q=I(b, B, c, C, d, D, t); 
  return I(a, P, d, Q, t);
} // P(a)=A, P(b)=B, P(c)=C, P(d)=D
pt I(float a, pt A, float b, pt B, float c, pt C, float d, pt D, float e, pt E, float t) {
  pt P=I(a, A, b, B, c, C, d, D, t); 
  pt Q=I(b, B, c, C, d, D, e, E, t); 
  return I(a, P, e, Q, t);
}
// Interpolation proportional to Distance
pt D(pt A, pt B, pt C, float t) {
  float a=0, b=d(A, B), c=b+d(B, C); 
  return I(a, A, b, B, c, C, a+t*(c-a));
}
pt D(pt A, pt B, pt C, pt D, float t) {
  float a=0, b=d(A, B), c=b+d(B, C), d=c+d(C, D); 
  return I(a, A, b, B, c, C, d, D, a+t*(d-a));
}
pt D(pt A, pt B, pt C, pt D, pt E, float t) {
  float a=0, b=d(A, B), c=b+d(B, C), d=c+d(C, D), e=d+d(D, E); 
  return I(a, A, b, B, c, C, d, D, e, E, a+t*(e-a));
}
